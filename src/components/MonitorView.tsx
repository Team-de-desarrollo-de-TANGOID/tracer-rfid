...
export default function MonitorView({
  ...
  const pushLine = useCallback((text: string, kind: LogLine['kind'] = 'log') => {
    ...
    setLines((prev) => [
      ...prev.slice(-499),
      ...parts.map((part) => ({ id: nextLineId(), text: part, kind })),
    ]);
  }, []);

  const applySnapshot = useCallback((snap: FxMonitorSnapshot) => {
    setSnapshot(snap);

    if (snap.logs?.lines) {
      const trimmed = snap.logs.lines.trim();
      if (trimmed) {
        setLines((prev) => [
          ...prev.slice(-499),
          ...trimmed
      ...
      if (snap.logOffset != null) logOffset.current = snap.logOffset;
    }
  ...
  const startStream = useCallback(() => {
    ...
    api.subscribeSyncMonitor({
      ...
      onClose: () => {
        const stale = Date.now() - lastSnapshotAt.current > 12_000;
        if (stale) setStreamActive(false);
      },
    });
  }, [applySnapshot, pushLine]);
  ...
  useEffect(() => {
    pushLine('Monitor iniciado — conectando al lector…', 'system');
    startStream();
  ...
  const handleRefresh = async () => {
    try {
      const snap = await api.syncMonitorSnapshot(logOffset.current);
      lastSnapshotAt.current = Date.now();
      setStreamActive(true);
      applySnapshot(snap);
    } catch (e) {
  ...
  const receiving =
    !paused && (streamActive || (lastSnapshotAt.current > 0 && Date.now() - lastSnapshotAt.current < 15_000));
  ...
  const toolbar = (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <button type="button" onClick={handleRefresh} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm rounded-lg border border-slate-200 bg-white hover:bg-slate-50">
        <RefreshCw className="w-4 h-4" />
        Actualizar
      </button>
      <button type="button" onClick={() => setLines([])} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm rounded-lg border border-slate-200 bg-white hover:bg-slate-50">
    ...
    >
      ...
      <div className="flex flex-wrap gap-2 shrink-0">
        ...
        >
          ...
          {receiving ? 'En vivo' : paused ? 'Pausado' : apiOk ? 'Conectado' : 'Desconectado'}
        </span>
      ...
      <div className="flex-1 min-h-0 flex flex-col rounded-xl border border-slate-200 bg-slate-900 overflow-hidden shadow-inner">
        <div className="flex items-center justify-between px-3 py-2 bg-slate-800 border-b border-slate-700 shrink-0">
          <span className="text-xs text-slate-400 flex items-center gap-1.5">
            ...
            /tmp/racketclub-gate.log
          </span>
          <label className="flex items-center gap-2 text-xs text-slate-400 cursor-pointer">
            <input type="checkbox" checked={autoScroll} onChange={(e) => setAutoScroll(e.target.checked)} className="rounded border-slate-600" />
            Auto-scroll
          </label>
        </div>
        <div className="flex-1 overflow-y-auto p-3 font-mono text-xs leading-relaxed">
          {lines.length === 0 ? (
            <p className="text-slate-500">Esperando datos del lector…</p>
          ) : (
            lines.map((line) => (
              <div
                key={line.id}
